...
ECHO waitting for devices...
adb -s %DeviceId% wait-for-device devices | findstr /r /C:"device$" 1>nul 2>nul && adb -s %DeviceId% logcat -b all -D >> "%~dp0\logcat_%data_time%.log"
GOTO LogcatLoop
...
IF "%debug%"=="true" ECHO DeviceId = %DeviceId%
ECHO waitting for devices...
adb -s %DeviceId% wait-for-device shell getprop ro.build.type 2>&1 | findstr /r /C:"^user$" 1>nul 2>nul || adb -s %DeviceId% logcat -b kernel >> "%~dp0\kernel_%data_time%.log"
adb -s %DeviceId% devices | findstr /r /C:"device$" 1>nul 2>nul && adb -s %DeviceId% shell getprop ro.build.type 2>&1 | findstr /r /C:"^user$" 1>nul 2>nul && adb -s %DeviceId% shell cat /proc/kmsg >> "%~dp0\kernel_%data_time%.log"
GOTO KernelLogLoop
...
adb -s %DeviceId% shell ls -l /d/tzdbg/qsee_log 2>&1 | findstr /r /C:" /d/tzdbg/qsee_log$" 1>nul 2>nul && TITLE qsee log %iDate% %iTime:~,8% && adb -s %DeviceId% shell cat /d/tzdbg/qsee_log >> "%~dp0\qsee_%data_time%.log"
GOTO TeeLogLoop
